...
class TestCleaningsRoutes:
    """ Test that Cleaning route api is created """
    async def test_routes_exist(self, app: FastAPI, client: AsyncClient) -> None:
        """ Test that path to create-cleaning exists """
...
class TestCreateCleaning:
    ...
    @pytest.mark.parametrize("invalid_payload, status_code",
        (
            (None, 422),
            ({}, 422),
            ({"name": "test_name"}, 422),
    ...
    async def test_invalid_input_raises_error(
        ...
        """ Multiple Tests with invalid input using pytest parametrize method """
        res = await client.post(
...
class TestGetCleaning:
    ...
    async def test_wrong_id_returns_errors(
    ...
    async def test_get_all_cleanings_returns_valid_response(
        ...
        # len of returned json would depend on what tests have been run before
        # since database persist for the test sessions
        assert len(res.json()) > 0
        cleanings = [CleaningInDB(**listing) for listing in res.json()]
...
class TestDeleteCleaning:
    async def test_can_delete_cleaning_successfully(
        ...
        assert res.status_code == HTTP_200_OK
        # ensure that the cleaning no longer exsits
        res = await client.get(app.url_path_for("cleanings:get-cleaning-by-id", id=test_cleaning.id))
        assert res.status_code == HTTP_404_NOT_FOUND
    ...
    async def test_can_delete_cleaning_unsuccessfully(
