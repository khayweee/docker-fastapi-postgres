...
from app.models.cleaning import CleaningCreate, CleaningPublic, CleaningInDB 
...
@router.get("/list_all", name="cleanings: List All Cleanings")
async def list_all(
    cleaning_repo: CleaningsRepository = Depends(get_repository(CleaningsRepository))
    ) -> List[CleaningInDB]:
    list_cleaning = await cleaning_repo.list_cleaning()
